...
def compute_fliper_exact(data_f : np.ndarray,kwargs : Dict) -> Union[float,None]:
    ...
    Fliper_20_d = FLIPER().Fp_20_days(data_f_new.T, mag)
    Fliper_80_d = FLIPER().Fp_80_days(data_f_new.T, mag)
...
def compute_nu_max(data: np.ndarray, f_flicker: float, kwargs: Dict) -> Tuple[float,Dict[str,float],Union[float,None]]:
    """
